...
// MARK: - RequestSignAccountIdProvider

class RequestSignAccountIdProvider: RequestSignAccountIdProviderProtocol {
    ...
    func getAccountId(completion: @escaping (String?) -> Void) {
        guard let mainAccount = self.userDataManager.getMainAccount()
        else {
            completion(nil)
            return
        }
        
        guard let walletData = self.userDataManager.getWalletData(account: mainAccount)
        else {
            completion(nil)
            return
        }
        
        completion(walletData.accountId)
    }
}
